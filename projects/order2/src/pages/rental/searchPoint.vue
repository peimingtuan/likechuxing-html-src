  ...
  <div class="SearchPoint">
    <div class="search-con clearfix">
      <div class="input-con float-left">
        <input type="text" @input="searchKeyword" v-model="keyword" placeholder="请输入还车的目的地" class="input" />
      </div>
    ...
    methods:{
      ...
      async selectBranch(){
        ...
        if(step === 'rental'){
          ...
          await this.$store.dispatch('rental/modifyEndBranch',this.active_branch)
          await this.$store.dispatch('rental/getCurrentInfo')
        }
        this.$router.go(-2)
      },
      selectPoint(poi){
        this.$store.commit('branch/setFlagLoc',poi)
        this.$router.go(-1)
      }
    }
  }
  ...
  .SearchPoint{
    ...
    .result-con{
      ...
      .icon-con{
        ...
        &.start{
          background-image: url(../../assets/images/search/搜索同取车网点@3x.png);
        }
        &.add{
          background-image: url(../../assets/images/search/搜索网点@3x.png);
        }
      }
      .name,.address{
        text-align: left;
