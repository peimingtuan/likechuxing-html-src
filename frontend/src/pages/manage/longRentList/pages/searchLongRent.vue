    ...
    <div class="container">
        <div class="header">
            <div class="input-wrap">
                ...
                <input type="text" v-model="kw" @input="search" placeholder="请输入网点名称">
        ...
        <div class="list" v-show="list.length">
            <div class="item" v-for="(item,index) in list" :key="index" @click="press(item.id)">{{item.name}}</div>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods:{
        ...
        postRequest(){
            ...
            const _this = this;
            http.post(api('/long-rental/search'),data).then(res=>{
                if(res.status===0){
                    _this.list = res.data
                }else{
                    Toast(res.msg)
