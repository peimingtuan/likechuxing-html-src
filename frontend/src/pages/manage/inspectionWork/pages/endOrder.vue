...
<script>
    ...
    mounted () {
        ...
        if(this.$route.query.branch_id && this.$route.query.name){//是否来自网点搜索页面
            ...
            this.search_id=this.$route.query.branch_id;
            this.searchWord=this.$route.query.branch_name;
            this.isnone=true;
            $(".floors").val(this.$route.query.begin_parking_floor);
            $(".carposinum").val(this.$route.query.begin_parking_no);
        }
    ...
    goMap(){
            window.location.href = '../endwork/index.html#/selectmapnet?type=6'
    },
    //提交信息
    submitInfo(){
            let _this=this;
            ...
            function postshow(param) {
                $.ajax({
                    type: "post",
                    data: param,
                    url: getApiUrl('/vehicle-inspection/operate'),
                    beforeSend: function () {
                        ...
                        if (data.status == '0') {
                            ...
                            setTimeout(function () {
                                _this.$router.push({
                                    path:'/finshcarwork',
                                    query:{
                                        id:sessionStorage.getItem("order_id")
                                    }
                                })
                            }, 2000)
