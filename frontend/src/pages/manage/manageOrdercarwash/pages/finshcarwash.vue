...
<script>
    ...
    mounted()
    {
        ...
        var arrytrailmany = [],//存放轨迹数组
                starticon = [],//起点
                endicon = [];//终点
        myAjax.post(getApiUrl('/vehicle-wash/gps'),{
            mobile:sessionStorage.getItem("mobile"),
            id: sessionStorage.getItem('carwash_orderid'),//洗车工单id
        }).then(data=>{
            if (data.status == '0') {
                $(".content-top .span3").text(data.data.coord_km+'KM')
        ...
        $(".select-net").on("click", function () {
            window.location.href = "./index.html#/selectmapnet2";
        });
        //结束工单后返回首页
