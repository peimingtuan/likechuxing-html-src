<template>
    <div class="container">
        ...
        <formCell label="定损金额" required="true">
            <div slot="inputbox" class="cell-item" >
                <input type="text" v-model="loss_money"  placeholder="请输入定损金额" class="item-input-box">
            </div>
        </formCell>
...
export default {
    ...
    computed:{
        ...
        loss_money:{
            get(){
                return this.$store.state.loss.loss_money
            },
            set(val){
                this.$store.commit('loss/setLossMoney', val)
            }
        },
        loss_no:{
            get(){
    ...
    methods:{
        ...
        createLoss(){
            ...
            if(this.loss_money===''){
                ...
                let num = parseFloat(this.loss_money)
                if(isNaN(num)){
                    Toast('定损金额格式不正确')
                    return 
                }
    ...
    beforeRouteLeave(to,from,next){
        if(to.path==='/workListDetail'){
            this.$store.commit('loss/emptyData')
        }
        next()
    },
    filters:{
