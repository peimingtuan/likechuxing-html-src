...
export default {
    ...
    computed:{
        ...
        isActvie(){
            let imgLength = this.$store.state.loss.accidentImg.length
            if(imgLength > 0){
                return true
            }else{
                return false
    ...
    methods:{
        getImage(){
            ...
            // }
            // arr.push(arrItem)
            // this.$store.commit('loss/setAccidentImg',arr)
            let self = this;
            dd.uploadImage({max:1}).then(res=>{
                ...
                arr.push(arrItem)
                self.$store.commit('loss/setAccidentImg',arr)
            }).catch(err=>{
                console.log(err)
            })
        },
        ...
        postUrl(){
            if(this.isActvie){
                ...
                this.$store.commit('loss/setAccidentImgUrl',arr)
                this.$router.go(-1)
            }
        }
    }
