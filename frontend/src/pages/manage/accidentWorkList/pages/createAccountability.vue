<template>
    <div class="container">
        ...
        <div class="row">
            <formCell label="处理方式" required="true">
                <div class="checkbox" slot="inputbox">
                    <div class="item" v-show="item.id!==2" :class="{'on':deal_mode==item.id,'big':index===typeList3.length-1}"  v-for="(item,index) in typeList3" :key="index" @click="selectType(item.id,'deal_mode')">{{item.name}}</div>
                </div>
            </formCell>
        </div>
        <div class="row">
        ...
        <div class="footer">
            ...
            <div class="comfirm-btn" @click="createAccountability">确定</div>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    data(){
        return{
            ...
            typeList3:[{id:1,name:'保险'},{id:2,name:'分时保险'},{id:3,name:'自费维修'},{id:4,name:'自费暂时不维修'}],
    ...
    created(){
        ...
        this.getDCarBelong(this.$store.state.workDetail.info.car_id)
    },
    methods:{
        ...
        selectType(id,type){
            switch(type){
                ...
                case 'response_belonging':
                    ...
                    break;
                case 'deal_mode':
                    this.$store.commit('accountability/setDeal_mode',id)
                    break;
                case 'detain':
        ...
        createAccountability(){
            ...
            this.createAccident(this.$router)
        },
        goPage(){
    ...
    beforeRouteLeave(to,from,next){
        if(to.path==='/workListDetail'){
            this.$store.commit('accountability/emptyData')
        }
        next()
    },
}
