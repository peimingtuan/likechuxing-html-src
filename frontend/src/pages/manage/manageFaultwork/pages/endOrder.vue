<template>
    <div id="endorder">
        ...
        <!-- 故障处理备注-->
        <div class="deal-depict">处理过程描述：</div>
        <div class="deal-remark-info">
            <div class="deal-remark-info2">
                <textarea class="void_writeinfo2"  placeholder="请描述您是如何处理的故障,3-200字(必填)" maxlength="200">
                    </textarea>
            </div>
        </div>
        <!-- 车辆状态备注-->
...
<script>
    ...
    mounted () {
        ...
        }else if(this.$route.query.is_rental){//来自蓝牙故障或租赁中车辆
    ...
    submitInfo(){
            ...
            let param={
                ...
                params: {
                    ...
                    operate_remark:$(".void_writeinfo2").val().replace(/(^\s*)|(\s*$)/g, ""),//处理故障说明，去除前后空格
                    remark: '',//车辆状态备注信息
                    status: $(".car-stateid").val() //车辆状态id
                }
            }
            if(_this.$route.query.is_rental){
                if($(".void_writeinfo2").val().replace(/(^\s*)|(\s*$)/g, "")==''|| $(".void_writeinfo2").val().replace(/(^\s*)|(\s*$)/g, "").length<3){
                    _this.$_LIKE_toast("请填写处理故障描述,不少于3字")
                    return false;
                }
            }else{
                if(_this.search_id==''){
                ...
                if ($(".car-stateid").val() == '33' || $(".car-stateid").val() == '25' || $(".car-stateid").val() == '23' || $(".car-stateid").val() == '20') {//状态为保养，保险，办公用车,车辆整备
                    ...
                    param.params.remark=$(".void_writeinfo").val().replace(/(^\s*)|(\s*$)/g, "");//车辆状态备注信息
                }
            }
...
<style>
    ...
    /*故障处理备注*/
    #endorder .deal-remark-info{
        background:#fff;
        padding:0 .12rem .12rem;
    }

    #endorder .deal-remark-info2 {
        width: 3.51rem;
        height: 1.4rem;
