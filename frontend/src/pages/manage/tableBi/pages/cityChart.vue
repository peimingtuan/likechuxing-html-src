    ...
    export default{
        ...
        methods:{
            getChartData(){
                ...
                return http.post(getAdminUrl('bi/city-operating'),tool.filterJson(data)).then((res)=>{
                    if(res.status==0){
            ...
            upDateData(param){
                ...
                const _this = this;
                return http.post(getAdminUrl('bi/city-operating'),tool.filterJson(_data)).then((res)=>{
                    if(res.status==0){
                        _this.xData = tool.fliterXData(res.data.xData)
                        _this.yData = tool.filterYData(res.data.yData)
                        _this.previewList = tool.initPreviewData(res).arr
            ...
            getCityList(){
                ...
                const _this = this;
                return http.post(getAdminUrl('bi/get-city'),tool.filterJson(data)).then((res)=>{
                    if(res.status==0){
                        let selectOptions = {}
                        let pickerData = res.data;
                        pickerData.city.unshift('全部')
                        pickerData.channel.unshift('全部')
                        selectOptions.city = [
                            {
                                values:pickerData.city
                            }
                        ]
                        selectOptions.channel = [
                            {
                                values:pickerData.channel
                            }
                        ]
                        _this.cityList = selectOptions.city
                        localStorage.setItem('selectDataList',JSON.stringify(selectOptions))
                    }
                }).catch(err=>{
                    console.log(err)
                })
