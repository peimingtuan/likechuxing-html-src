<template>
    <CarBox  :data="data.car">
        <div class="maintain-box">
            ...
            <div class="item"><span class="label">已完成保养次数：</span>{{data.last_maintain_number || 0}}</div>
            <div class="item"><span class="label">上次保养时已行驶总公里（里程表）：</span>{{data.last_custom_km || '-'}} 公里</div>
            <div class="item"><span class="label">上次保养时间：</span>{{data.last_maintain_time || '-'}}</div>
            <div class="item"><span class="label">上次保养类型：</span>{{data.last_maintain_type || '-'}}</div>
        </div>
    </CarBox>
</template>
