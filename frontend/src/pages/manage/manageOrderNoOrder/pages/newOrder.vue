...
<template>
	<div class="newOffice">
		<div class="list_cont move_item">
			...
			<div class="status_modal">
				<div class="status_title">
					<p 
						class="move_status_zero"
					>
						<span 
							class="line_left line_left_zero"
						></span>
						待追车
					</p>
					<p class="fc66">
    	...
    	<p class="with_peo" @click="goAdd">
    		<span>
    			随行人员：
    			<i class="fc99" v-if='names.length==0'>如有随行人员，请添加</i>
    		</span>
	...
	export default{
		...
		methods: {
			...
			createSub(){//提交
				...
				myAjax.post(getOspApiUrl('/vehicle-abnormal/create-order'), {
				    car_id: this.car_id,
				    order_id: this.listItem.id,
				    remark:this.creatNote||'',
				    mobile:userData.state.mobile,
				    again_person:this.personId.join(','),
				}).then(res => {
				  	if (res.status === 0) {
					  		this.$router.push({
					          path:'/orderDetail',
