...
<template>
	<div class="serviceMap">
		<div class="dispatch-search" v-show="!isData">
		    <search 
		    	v-if = 'type == 4'
				ref='search'
				:search_name="''"
		...
		<div v-if="type==4&&!isData" class="check_btn"  @click="goEnd()">
			确认选择
		</div>
		<div v-else-if="type!=4&&!isData" :class="isnext||type=='2'||type=='3'?'check_btn':'bgcff'" @click="goEnd()">
	...
	export default {
	    ...
	    data () {
	      return {
	        ...
	        choice_floor:'',
	        mapResult:[],
	        searchLng:'',//高德搜索返回经纬度及name，branch
	        searchLat:'',
	        searchName:'',
	        searchBranch:'',
	        isData:JSON.parse(sessionStorage.getItem('mapData'))||'',
	      }
	    },
	    mounted () {
	    	this.creatMap();
	    ...
	    methods: {
	    	...
	    	creatMap(){
		        ...
		        //初始化加载数据
		            ...
		            this.bigicon = [];//存放大图标
		            if(this.isData){
		           	 	this.positionPick(this.isData.lng,this.isData.lat)
		            }else{
		           	 	this.positionPick(userData.state.locationsLng,userData.state.locationsLat)
		            }
		        }).catch(err => {
	    		...
	    		AMapUI.loadUI(['misc/PositionPicker'], function (PositionPicker) {//拖动定位图标
	                ...
	                positionPicker.on('success', function (positionResult) {
	                    if(_this.type!=4){
	                    	...
	                    	var search = _this.$refs.search.searchData;
	                    	if(!search){
	                    		_this.mapData(positionResult);
	                    		_this.$refs.search.searchWord = '';
	                    	}else{
	                    		_this.$refs.search.searchData = '';
	                    	}
	                    }
	                });
	    	...
	    	mapData(branchData){//处理搜索高德地图位置的返回数据
	    		var name ,address,lng,lat,_this = this;
	    		if(this.$refs.search.searchData){
	    			this.branchData = this.$refs.search.searchData;
	    			name = this.branchData.name;
		    		address = this.branchData.address;
		    		lng = this.branchData.location.lng;
		    		lat = this.branchData.location.lat;
		    		this.positionPick(lng,lat)
	    		}else{
	    			if(this.isData){
		    			name = this.isData.name;
			    		address = this.isData.address;
			    		lng = this.isData.lng;
			    		lat = this.isData.lat;
	    			}else{
		    			this.branchData = branchData;
		    			name = this.branchData.nearestPOI;
			    		address = this.branchData.address;
			    		lng = this.branchData.position.lng;
			    		lat = this.branchData.position.lat;
			    	}
	    		}
	    		$(".navition-bottom .net").text(name);
                $(".navition-bottom .address").text(address);
                $(".navition-bottom .lng").val(lng);
                $(".navition-bottom .lat").val(lat);
                $('.serviceMap .current_location2').css('bottom','1.6rem')
                $('.serviceMap .refure-dispatchpage').css('bottom','2.15rem')
                this.navition = true;
                this.searchLng = lng;
                this.searchLat = lat;
                this.searchName = name;
                this.searchBranch = address;
                //导航,需要提前构造
	           _this.driving;
	    		...
	    		myAjax.post(getOspApiUrl('/work-order/select-branch'), param).then(res => {
	                    ...
	                    if (res.status == '0') {
	                        ...
	                        if (result.length != '0') {
	                        	...
	                        	_this.isnext = false;
	                        	$('.serviceMap .current_location2').css('bottom','.65rem')
	                        	$('.serviceMap .refure-dispatchpage').css('bottom','1.15rem')
	                            for (var i = 0; i < result.length; i++) {
	                                let netpoint = [result[i].lng, result[i].lat];//当前网点位置信息
	                                _this.Array.push(netpoint);
	                                addMarker(netpoint, result[i].name, result[i].address, result[i].car_num,result[i].branch_id,result[i].biz_type,result[i].choice_floor);//小图标
	                                addMarkerbig(netpoint, result[i].name, result[i].address, result[i].car_num,result[i].branch_id,result[i].biz_type,result[i].choice_floor);//大图标
	                            }
	                            //map.setFitView();
	                        } else {
	                    ...
	                    // 编写自定义函数,创建小图标标注
	                    function addMarker(netpoint, branch_name, address, lin_recommend_car,branch_id,biz_type,choice_floor) {
	                        let classtype = '';
	                        if (biz_type == '0') {
	                        ...
	                        if(_this.branch_id==branch_id){
	                            ...
	                            _this.choice_floor = choice_floor;
	                        }
	                    ...
	                    // 编写自定义函数,创建大图标标注
	                    function addMarkerbig(netpoint, branch_name, address, lin_recommend_car,branch_id,biz_type,choice_floor) {
	                        let classtype = '';
	                        if (biz_type == '0') {
	    	...
	    	goEnd(){
	    		...
	    		if(this.isnext&&this.type!=2&&this.type!='3'){
	    			window.location.href="index.html#/endOrder?status_note="+this.$route.query.status_note+"&branch_id="+this.next_id+"&branch_name="+this.next_name+'&order_id='+this.$route.query.order_id+'&choice_floor='+this.choice_floor+'&type='+this.$route.query.noorder+'&car_id='+this.$route.query.car_id
	    		}else if(this.type==2){
	    			...
	    			var datas = {
	                	...
	                	"address":this.searchBranch,
	                }
	                sessionStorage.setItem('mapData',JSON.stringify(datas));
	    			window.location.href="index.html#/addCarinfo?order_id="+this.$route.query.order_id
	    		}
	    	}
	    },
  	}
