		...
		<div class="getpicture-content">
		    ...
		    >
		        ...
		        <div>
		            ...
		            <span class="num" v-show="photo.num">{{num}}/10</span>
	    ...
	    methods: {
	    	...
	    	getPicture(photo,e,index){
			    ...
			    dd.uploadImageFromCamera({
			        ...
			        if(photo==1){
			        	that.num++;
			        	if(that.type==1&&that.partsNum.length<10){
							that.allPhoto.push({'title':'维修部位','top':true,'num':1})
						}else if(that.type==2&&that.num<=10){
							that.allPhoto.push({'title':'维修部位','top':true,'num':1})
						}
			        }
			...
			deletePhone(num,item,e){
				...
				if(num==1){
					...
					if(this.num==10)this.allPhoto.push({'title':'维修部位','top':true,'num':1})
