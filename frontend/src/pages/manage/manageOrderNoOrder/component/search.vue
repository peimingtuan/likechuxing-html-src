	...
	<div class="search">
		<div class="repair_shop">
			...
			<span class="repair_sear">
				<input type="text" name="searchWord" @focus="hisData()" v-model='searchWord' :placeholder="plac_txt" @input.trim="keyupall(searchWord)"/>
		...
		<div class="history_items" v-show="issucc||ishis">
			<div>
				<p v-if='result&&result.length==0&&ishis&&search_type!=4' class="his_title">历史记录：</p>
				<p v-if='result&&result.length==0&&ishis&&search_type!=4' class="his_list" 
				...
				<!--搜索高德地图本身-->
				<p v-if="search_type==4"
					class="history_item" 
					v-for="(item,index) in mapResult"
					:key = 'index'
					data-type = 'index' 
					@click="goMap(index,item)">
					{{item.name}}
				</p>
...
//	    	mapResult:''
	    ...
	    methods: {
	    	...
	    	keyupall(word){//搜索结果事件
				...
				if(this.searchWord&&this.searchWord.length>0&&this.search_type!=4){
					var param = {
				...
				}else if(this.searchWord&&this.searchWord.length>0&&this.search_type==4){
					var _this = this, keywords;
					setTimeout(()=>{
					    ...
					    AMap.plugin('AMap.PlaceSearch', function(){
					        ...
					        var placeSearchs = new AMap.PlaceSearch(autoOptions);
					        placeSearchs.search(keywords, function(status, result) {
			...
			goMap(index,item){//搜索高德地图本身
				this.searchWord = item.name;
				this.isnone = false;
				this.issucc=false;
				this.ishis=false;
				this.searchData = item;
				this.$emit('mapData',true);
			},
