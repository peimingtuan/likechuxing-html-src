<!--
	...
	allInfo:{'plate_no':'粤AC6666','car_info':'北汽','total_km':'23',
			'addrs':[{'icon_color':'1','icon_img':'丢','branch_name':'蓝牙测试网点','time':'2018-10-24  10:10'},
				{'icon_color':'2','icon_img':'追','branch_name':'蓝牙测试网点','time':'2018-10-24  10:12'},
...
<template>
	<div class="end">
		<div id="container"  v-if='allInfo.catch_coord_list||allInfo.end_coord_list'>
		</div>
        ...
        <div class="end_content">
			...
			<div class="addrs">
				<div class="addrs_item"  
					...
					:key = 'index'
					>
					<span :class="item.icon_color==1?'icon_55':item.icon_color==2?'icon_yellow':'icon_red'">{{item.icon_img}}</span>
					<div>
	    	...
	    	inits(){
			        let arrytrail = [],
                     ...
                     var arrytrailmany = [],//存放轨迹数组
			                ...
			                chargeicon=[],//维修站
			                endicon = [];//终点
                    var end = this.allInfo.end_coord_list,catch_car = this.allInfo.catch_coord_list;
                   if(catch_car||end){
	                   	for (var i = 0; i < catch_car.length; i++) {
	                        lng = catch_car[i].lng;
	                        lat = catch_car[i].lat;
	                        arrytrail = [lng, lat];
	                    ...
	                    if(chargeicon.length!=0){
	                        let chargemark = new AMap.Marker({//充电站
	                            ...
	                            icon: new AMap.Icon({
	                                size: new AMap.Size(25, 30),  //图标大小
	                                image: require('../images/give_icon.png'),
	                                imageSize: new AMap.Size(23, 30)
	                            })
	                        });
	                    }
