...
require('../css/oilerSingle/searchnetwork.css')
import $ from 'jquery'
        ...
        $.post(getApiUrl('/work-order/search'), param, function (data) {
            if (data.status == '0') {
                ...
                for (var i = 0; i < data.data.length; i++) {
                    str += '<li class="record">' + data.data[i].name + '<input type="hidden" value="' + data.data[i].name + '" class="name" /><input type="hidden" value="' + data.data[i].branch_id + '" class="branch_id" /><input type="hidden" value="' + data.data[i].lng + '" class="lng" /><input type="hidden" value="' + data.data[i].lat + '" class="lat" /></li>';
                }
                $(".searchnetwork-list").html(str);
            } else {
                new Toast(data.msg)
    ...
    if ($(".search-car").val() == '') {
        ...
        $.post(getApiUrl('/work-order/search'), param, function (data) {
            ...
            if (data.length == 0) {
                ...
                if (data.length == 1) {
                    if (type == 1) {
                        window.location.href = "./carwoundpicture.html?name=" + data[0].name + "&branch_id=" + data[0].branch_id + "&lng=" + data[0].lng + "&lat=" + data[0].lat + "&type=1";
                    } else {
                        window.location.href = "./carwoundpicture.html?name=" + data[0].name + "&branch_id=" + data[0].branch_id + "&lng=" + data[0].lng + "&lat=" + data[0].lat;
                    }
                } else {
                    let str = '';
...
$(".searchnetwork-list").on('click', 'li', function () {
    ...
    if ($(this).hasClass("winding")) {
        ...
        $.post(getApiUrl('/work-order/search'), param, function (data) {
            ...
            if (status == '0') {
                if (type == 1) {
                    window.location.href = "./carwoundpicture.html?lng=" + param.lng + "&lat=" + param.lat + "&branch_id=" + param.branch_id + "&type=1";
                } else {
                    window.location.href = "./carwoundpicture.html?lng=" + param.lng + "&lat=" + param.lat + "&branch_id=" + param.branch_id;
                }
            } else {
                new Toast(msg)
            }
