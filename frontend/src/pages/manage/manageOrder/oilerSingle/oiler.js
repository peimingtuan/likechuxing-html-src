...
require('../css/oilerSingle/oiler.css')
import $ from 'jquery'
...
function show(param) {
    ...
    $.post(getApiUrl('/work-order/mine'), param, function (data) {
        ...
        if (status == '0') {
            ...
            if(recordrentList.data.length==0){
                ...
                for (var i = 0; i < recordrentList.data.length; i++) {
                    ...
                    str += '<li class="carDetail"><input type="hidden" value="' + recordrentList.data[i].city_id + '" class="city_id"/><input type="hidden" value="' + recordrentList.data[i].id + '" class="workId"/><input class="carworkstate" type="hidden" value="' + recordrentList.data[i].status + '"/><span>' + recordrentList.data[i].plate_no + ' （<font class="font1">'  + strstart + '</font>'+strend +'）</span><span>加油'+recordrentList.data[i].id+'</span>' +
                        '<span>' + recordrentList.data[i].status_name + '</span><span>' + recordrentList.data[i].car_brand + '</span></li>';
                }
            }
            $(".oilerList").html(str);
            //上拉，下拉获取数据
...
//初始化加载图片
$(".close-oiler img").attr("src", require('../image/close.png'));
//跳转搜索页
