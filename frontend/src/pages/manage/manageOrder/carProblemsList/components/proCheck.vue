	...
	<div class="proCheck">
		...
		<div class="check_modal" v-show="isModal">
			<div class="modal_content">
				...
				<div class="text_dec">
					<input v-model="textarea1" id="textarea" placeholder="*请添加描述，50字内（必填）" name="" maxlength ="50">
				</div>
				<div class="alert_btn">
	...
	export default{
		...
		mounted () {
			...
			if(car_id){
				...
				this.car_id = sessionStorage.getItem('car_id')
			}
			this.init();
		...
		methods: {
			...
			goAjax(param){
				...
				myAjax.post(getApiUrl('/vehicle-problem/create-report'), param, function (data) {
					if(data.status==0){
						window.location.href='../../manageOrderCardetail/rentcarlist.html'
					}else{
						_this.isShow = true;
				 		_this.msg = data.msg;
				 		setTimeout(()=>{_this.isShow=false;},2000)
