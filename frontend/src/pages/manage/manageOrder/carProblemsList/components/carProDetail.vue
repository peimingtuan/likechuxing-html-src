...
<template>
	<div class="carProDetail" ref='carProDetail'>
				...
				<div class="carpro_date">
					...
					<p class="car_history" @click="goHandle()">
						...
						<span class="icon_history">
							<img src="../../image/icon_right66.png" alt="" />
						</span>
					</p>
				</div>
			</div>
		...
		<div class="carpro_footer fc33">
			...
			<div v-if="btn" class="footer_btn build_btn" @click="goCreate()">
				创建故障工单
			</div>
			<div v-else class="footer_btn build_btn" @click="notdeal()">
				无需处理
			</div>
		</div>
	...
	export default{
		data () {
			return{
				...
				cleData:[],//弹窗数据展示内容
				listItem:JSON.parse(sessionStorage.getItem('resultData'))||JSON.parse(sessionStorage.getItem('listItem')),//获取缓存数据
		...
		beforeCreate () {
			if(this.$route.query.type==1)btn=false;
		},
		destroyed(){
			$('html').height('100%');
			...
			init(){
				...
				myAjax.post(getApiUrl('/vehicle-problem/problem-detail'), param, function (data) {
					if(data.status==0){
						...
						_this.listItem.status_num=_this.carsData.status;
			...
			confirBtn(){//确认按钮
				...
				if(this.$route.query.type==1){
					...
					myAjax.post(getApiUrl('/vehicle-inspection/operate'),{
						...
						if(data.status==0){
							window.location.href = "../../inspectionWork/index.html#/workdetail?id="+_this.$route.query.id
						}else{
							_this.isShow = true;
							_this.msg = data.msg;
							setTimeout(()=>{_this.isShow=false;},2000)
