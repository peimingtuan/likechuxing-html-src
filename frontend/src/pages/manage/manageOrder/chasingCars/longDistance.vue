	            ...
	            <mt-loadmore :top-method="loadTop" :autoFill=false :distance-index='1' @translate-change="translateChange" @top-status-change="handleTopChange" :bottom-method="loadBottom" @bottom-status-change="handleBottomChange" :bottom-all-loaded="allLoaded" ref="loadmore">
	            	<div class="move_item" v-for="(listItem,index) in moveData" @click = goDetail(listItem.plate_no,listItem.vin)>
						...
						<p class="stop_pays fc66">
 						...
 						<p class="not_time">
 							距离城市中心：
 							<span class='fcec6e'>{{listItem.city_distance}}
 							</span>KM
 						</p>
					...
					<div class="check_more">
						<div class="filter_dec">
							...
							<div class="fildec_cont">
								<span class="fildec_items filmore1 bg_white" id='0' @click="checkList('-1',$event)">全部车辆</span>
								...
								<!--<span class="fildec_items filmore" :id = 'items.id' :class="[items.id==1||items.id==2||items.id==103||items.id==102?'isOne':'notOne']" v-for="(items,index) in moreData" @click="checkList(items.id,$event)" v-if='limit_num==0'>{{items.name}}</span>
								<span class="fildec_items filmore" :id = 'items.id' :class="[items.id==1||items.id==2||items.id==103||items.id==102?'isOne':'notOne']" v-for="(items,index) in moreData" @click="checkList(items.id,$event)" v-else>{{items.name}}({{items.count}})</span>-->
							</div>
						</div>
					</div>
				</div>
	...
	export default{
		data () {
			return{
				...
				sort:[
					{"sortCont":"距离城市中心从远到近",'sortType':'0'},
			...
			dingding(){//钉钉授权等
				...
				dd.ready(function () {
					dd.biz.navigation.setTitle({
					    title : '远距离车辆',//控制标题文本，空字符串表示显示默认文本
...
//						_this.moveData = _this.moveData.unshift({'unmove_time':'1天4小时5分钟'})
			...
			goSearch(){//去搜索页面
				...
				window.location.href = './warnIndex.html#/searchPro?catType=1';
			},
			filClose(){//关闭筛选层
