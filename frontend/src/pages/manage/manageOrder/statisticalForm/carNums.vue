...
<template>
	<div class="carNums">
		<div class="four_header fc33">
			...
			<div class="note_out">
				<p class="form_note" @click="noteShow=true">统计说明</p>
			</div>
		</div>
		<!--<div class="four_content fc33" v-show="statusData.length>0">--> 
		<div class="four_content fc33"> 
	            	<div class="form_item" v-for="status_item in statusData">
	            		<p class="car_num_title">
	            		...
	            		<ul class="ul_carlist">
	            			...
	            			<li class="car_status" v-for = '(detItem,index) in status_item.detail' @click="goDetail(detItem,status_item.city_name,status_item.city_id)">
					...
					<div class="check_more">
						<div class="filter_dec">
							...
							<div class="fildec_cont">
								<span class="fildec_items filmore1 isOne" id='0' @click="checkList('-1',$event)">全部</span><span class="fildec_items filmore notOne" :id = 'items.id'  v-for="(items,index) in moreData" @click="checkList(items.id,$event)">{{items.name?items.name.substring(0,items.name.length-1):''}}</span>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="check_modal note_modal fc33" v-show="noteShow">
			<div class="note_content">
				...
				<p class="note_item fc66" v-for='item in noteData'>
					{{item}}
				</p>
	...
	export default{
		data () {
			return{
				...
				moreData:'',//筛选数据
		        islimit:'',
		        noteData:["1.所有车辆状态均为实时统计；","2.空闲占比=空闲/（空闲+租赁中）；","3.租赁中占比=租赁中/（空闲+租赁中）；","4.运营车辆=空闲+租赁中+下线；","5.上线率=运营车辆数/（总数-整备-办公用车）；","6.车辆整备占比=整备车辆数/该城市所有车辆；","7.除空闲、租赁中、整备外其他各状态占比=状态数量/（该城市所有车辆-车辆整备数量）。"],//调度说明需求
				//假数据
				statusData:'',
		...
		methods:{
		    ...
		    goDetail(details,city_name,city_id){
		    	...
		    	if(details.click==1){
		    		window.location.href = './formIndex.html#/carDetails?details='+det+'&cityName='+city_name+'&cityId='+city_id;
		    	}
