		...
		<div class="history_items" v-show="issucc">
			<div>
				<p class="history_item" 
					...
					:key = 'index'
					data-type = 'index' 
					@click="goDec(index,item.id,item.plate_no,item.vin,item.update_time,item.name,item.biz_type,item.lat,item.lng,item.data_type,item.branch_id,item.limit_num,item.area)">
	...
	export default{
		...
		methods:{
			...
			goDec(resType,resId,chepai,vin,update_time,name,biz_type,lat,lng,data_type,branch_id,carNum,area){//搜索后判断类型进行页面跳转
				...
				//清除列表页筛选等条件
				if(data_type==0){
					window.location.href = './index.html#/notMove?car_id='+resId+'&search=true';
				}else{
					window.location.href = './index.html#/notMove?branch_id='+branch_id+'&search=true';
				}
			},
