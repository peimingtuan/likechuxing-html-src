<template>
	<div id="selectmapnet">
		...
		<!-- 底部按钮-->
		<div class="sure-btn normal-color" v-if="isnext" @click="goendwork()">
			确认选择
		</div>
		<div class="sure-btn" v-else>
			请选择网点
		</div>
	</div>
</template>
...
<script>
	...
	methods: {
		...
		netshow(param){
			let _this = this,
					nozzleaddress = '/work-order/select-branch';//正常网点查询
			if(this.$route.query.type==3) {//更改网点,可查虚拟网点
				nozzleaddress='work-order/branch-info';
			}
			myAjax.post(getApiUrl(nozzleaddress), param).then(res => {
				...
				// 编写自定义函数,创建小图标标注
				function addMarker(netpoint, branch_name, address, lin_recommend_car,branch_id,biz_type,choicefloor) {
		...
		//确认选择
		goendwork(){
			if(this.$route.query.type==3){//更改网点
				window.location.href = "../manageOrder/commonFile/changenetstatus.html?branch_id="+this.next_id+"&name="+this.next_name;
			}else if(this.$route.query.type==4){//删除工单
				window.location.href = "../manageOrder/oilerSingle/deletework.html?branch_id="+this.next_id+"&name="+this.next_name;
			}else if(this.$route.query.type==5){//充电工单
				window.location.href = "../manageOrder/chargeWork/charge.html#/endchargework?branch_id="+this.next_id+"&name="+this.next_name;
			}else if(this.$route.query.type==6){//巡检工单
				window.location.href = "../inspectionWork/index.html#/endOrder?branch_id="+this.next_id+"&name="+this.next_name;
			}else{
				...
				this.$router.push({
					path:'/endOrder',
					query:{
						branch_id:this.next_id, //选择的网点id
						name:this.next_name, //选择的网点name
						type:this.type
					}
				})
			}
