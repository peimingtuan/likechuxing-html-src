	...
	export default{
		data () {
			return{
				...
				result:'',//所有数据
				arr:JSON.parse(localStorage.getItem('searchName'))||[],//历史搜索
				isHis:false,//是否是历史数据
				enter:'',//判断入口页面
				placeh:'',
		...
		methods:{
			...
			goDec(user_id){//搜索后判断类型进行页面跳转
				//清除列表页筛选等条件
				this.arr.push(this.searchWord);
				this.unique(this.arr);
//				window.location.href = 'index.html#/list?user_id='+user_id;
			...
			unique(allarr){//历史记录去重
				...
				this.arr = res;
				localStorage.setItem('searchName',JSON.stringify(this.arr));
			}
		}
	}
