		...
		<div class="his_content">
			...
			>
				...
				<div class="his_right">
					<div>
						<p>{{item.operate}}</p>
						<p class="fc99 his_name" v-if='item.item_id!=3&&item.item_id!=8'>
							<span>{{item.real_name}}</span>
							&nbsp;
							<span>{{item.user_name}}</span>
						</p>
						<p class="first_type type_one" v-if="item.item_id==1">办公类型：{{item.remark.order_type_text}}</p>
						<p class="first_type" v-if="item.item_id==1">随行人员：
							<span v-for="person in item.remark.persons"
								:key="person.id"
							>
							{{person.realname}}
							</span>
						</p>
						<p class="first_type" v-if="item.item_id==1">
							目的地和办公缘由：{{item.remark.info}}
						</p>
					...
					<p class="check" v-if="item.item_id==3||item.item_id==4||item.item_id==2||item.item_id==8" @click="goIndex(item.id,item.item_id)">
						<span>查看</span>
