...
export default {
    ...
    created(){
        ...
        let flag = localStorage.getItem('botao_new_user')
        if(flag==0){
            this.showErr = true
            this.show = true
        }else if(flag==1){
    ...
    methods:{
        getCode(){
            ...
            http.post(api('activity-botao/get-code'),data).then(res=>{
                if(res.status==0){
                    Toast('发送成功')
                }else{
                    Toast(res.msg)
        ...
        getCoupon(){
            ...
            let _this = this;
            http.post(api('activity-botao/verify-code'),data).then(res=>{
                if(res.status==0){
                    localStorage.setItem('botao_new_user',res.data.new_user)
                    ...
                    if(res.data.hasOwnProperty('new_user')){
                        _this.showErr = true
                        _this.show = true
                        localStorage.setItem('botao_new_user',0)
                    }else{
                        Toast(res.msg)
                    }  
                }
