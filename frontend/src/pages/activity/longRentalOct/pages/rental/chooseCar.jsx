...
class Prepare extends Reflux.Component {
    ...
    render() {
        let time = this.state.fetch_car_time ?<p style={{
            paddingTop:'0.1rem'
            }}>{new Time(this.state.fetch_car_time).getTime('YYYY 年 M 月 D 日 HH:00')}</p>:""
        return (
        ...
        if (this.state.car_list.length === 0) {
            ...
            return (
                <div className="Choose paddingStyle2">
                    ...
                    <p>您预定的车型：{this.state.model_name}</p>
                        {list}
                    </div>
                </div>
            )
...
export default class ChooseCar extends Reflux.Component {
    ...
    getTime(){
        let that = this
        myAjax.post(getApiUrl('/time/get'),{}, function (res) { 
            // console.log(res);
            that.setState({"server_time":JSON.parse(res).data.timestamp})
        })
    }
    cancel(){
        this.getTime();
        if(this.state.has_change_car) return;
